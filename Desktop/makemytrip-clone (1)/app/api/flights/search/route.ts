...
export async function POST(request: Request) {
  try {
    ...
    body.segments = body.segments.map((segment: any) => ({
      ...segment,
      PreferredDepartureTime: `${segment.PreferredDepartureTime.split('T')[0]}T00: 00: 00`,
      PreferredArrivalTime: `${segment.PreferredArrivalTime.split('T')[0]}T00: 00: 00`
    }));
    ...
    const response = await fetch('https://platform.edusession.live/easy2trip/api/flight/search', {
      method: 'POST',
      headers: {
